...
// berdasar `source_role` pencatat; backend Fase 1 tidak melacak siapa
// pencatatnya per transaksi lagi), disajikan sbg halaman terpisah supaya
// menu Staff tetap ringkas (tanpa Jurnal/Laporan lengkap).
function PenerimaanKasForm({ accounts, onDone }: { accounts: AccountDto[]; onDone: () => void }) {
  const { tt } = useI18n();
  ...
  async function simpan() {
    ...
    try {
      ...
      showToast(err instanceof ApiError ? err.message : 'Gagal menyimpan penerimaan kas.', 'error');
    }
  }

  return (
    <div className="mt-4 bg-dark-800 rounded-xl border border-gray-700/50 p-5">
      <h3 className="text-sm font-bold text-white mb-4">{tt('heading.tambahPenerimaanKasBaru')}</h3>
      <form onSubmit={(e) => { e.preventDefault(); simpan(); }} className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div><label className="text-[10px] text-gray-500 block mb-1">{tt('label.keteranganPenerimaan')} *</label><input type="text" required value={ket} onChange={(e) => setKet(e.target.value)} placeholder={tt('placeholder.contohKasKelas')} className="w-full bg-dark-900 border border-gray-700 rounded-lg px-3 py-2 text-xs text-gray-300 focus:outline-none focus:border-brand-500" /></div>
  ...
  return (
    <>
      <div className="print-sheet bg-dark-800 rounded-xl border border-gray-700/50 p-5">
        ...
        <PeriodFilterBar ns="penerimaan_kas" />
        {loading ? <p className="text-xs text-gray-500 py-4">Memuat...</p> : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tfoot><tr className="border-t border-gray-600 font-bold"><td colSpan={4} className="py-3 text-white">{tt('col.total')}</td><td className="py-3 text-right text-emerald-400">{fmt(masuk.reduce((a, t) => a + t.amount, 0))}</td><td></td></tr></tfoot>
            </table>
          </div>
        )}
      </div>
      {formOpen && <PenerimaanKasForm accounts={accounts} onDone={() => { setFormOpen(false); load(); }} />}
    </>
  );
}
