...
function PengeluaranForm({ accounts, onDone }: { accounts: AccountDto[]; onDone: () => void }) {
  const { tt } = useI18n();
  const { showToast } = useToast();
  const [ket, setKet] = useState('');
  const [jumlah, setJumlah] = useState(250000);
  ...
  async function simpan() {
    ...
    const amount = jumlah || 250000;
    try {
      await createTransaction({
        txDate: tgl || new Date().toISOString().split('T')[0], description: ket.trim(), txType: 'Keluar', paymentMethod: metode, amount,
        journalLines: [{ accountId: debitId, debit: amount, credit: 0 }, { accountId: creditId, debit: 0, credit: amount }],
      });
      showToast(`${tt('msg.pengeluaranSebesar')} ${fmt(amount)} ${tt('msg.berhasilDicatat')}`);
      onDone();
    } catch (err) {
      showToast(err instanceof ApiError ? err.message : 'Gagal menyimpan pengeluaran.', 'error');
    }
  }

  return (
    <div className="mt-4 bg-dark-800 rounded-xl border border-gray-700/50 p-5">
      <h3 className="text-sm font-bold text-white mb-4">{tt('heading.tambahPengeluaranBaru')}</h3>
      <form onSubmit={(e) => { e.preventDefault(); simpan(); }} className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div><label className="text-[10px] text-gray-500 block mb-1">{tt('label.keteranganPengeluaran')} *</label><input type="text" required value={ket} onChange={(e) => setKet(e.target.value)} placeholder={tt('placeholder.contohAtk')} className="w-full bg-dark-900 border border-gray-700 rounded-lg px-3 py-2 text-xs text-gray-300 focus:outline-none focus:border-brand-500" /></div>
        ...
        <div><label className="text-[10px] text-gray-500 block mb-1">{tt('label.tanggalPengeluaran')}</label><input type="date" value={tgl} onChange={(e) => setTgl(e.target.value)} className="w-full bg-dark-900 border border-gray-700 rounded-lg px-3 py-2 text-xs text-gray-300 focus:outline-none focus:border-brand-500" /></div>
        <div className="md:col-span-2"><JournalAccountPicker accounts={accounts} debitId={debitId} setDebitId={setDebitId} creditId={creditId} setCreditId={setCreditId} debitLabel="Akun Debit (Beban)" creditLabel={`Akun Kredit (${metode})`} /></div>
...
export function Pengeluaran() {
  ...
  const st = get('pengeluaran');

  async function load() {
    setLoading(true);
    try {
      const [t, a] = await Promise.all([fetchTransactions({ txType: 'Keluar' }), fetchAccounts()]);
      setTransactions(t); setAccounts(a.filter((x) => x.isActive));
    } catch (err) {
      showToast(err instanceof ApiError ? err.message : 'Gagal memuat data pengeluaran.', 'error');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      <div className="print-sheet bg-dark-800 rounded-xl border border-gray-700/50 p-5">
        ...
        <PeriodFilterBar ns="pengeluaran" />
        {loading ? <p className="text-xs text-gray-500 py-4">Memuat...</p> : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {keluar.length ? keluar.map((t) => (
                  <tr key={t.id} className="border-b border-gray-700/30 hover:bg-dark-850/50">
                    ...
                    <td className="py-3 text-right font-semibold text-red-400">{fmt(t.amount)}</td>
              ...
              <tfoot><tr className="border-t border-gray-600 font-bold"><td colSpan={4} className="py-3 text-white">{tt('col.total').toUpperCase()}</td><td className="py-3 text-right text-red-400">{fmt(keluar.reduce((a, t) => a + t.amount, 0))}</td><td></td></tr></tfoot>
            </table>
          </div>
        )}
      </div>
      {formOpen && <PengeluaranForm accounts={accounts} onDone={() => { setFormOpen(false); load(); }} />}
    </>
  );
}
